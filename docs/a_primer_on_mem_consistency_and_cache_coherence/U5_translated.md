...
$$
\text{ - All L1i, All S1j } \rightarrow  \mathrm{R}1 \rightarrow  \mathrm{A}2 \rightarrow  \mathrm{{All}}\mathrm{L}2\mathrm{i}\text{ , All S2j, }
$$
...
<table><tr><td>Core C1</td><td>Core C2</td><td>Comments</td></tr><tr><td>S1: data1 = NEW; <br> S2: data2 = NEW; <br> S3: flag = SET;</td><td>L1: r1 = flag <br> B1: if (r1 ≠ SET) goto L1; L2: r2=data1; <br> L3: r3=data2;</td><td>/* Initially, data1 and data2 = 0 & flag ≠ SET */ <br> /* spin loop: L1 & B1 may repeat many times */</td></tr></table>
...
<table><tr><td>Core C1</td><td>Core C2</td><td>Comments</td></tr><tr><td>A1: acquire(lock) /* Begin Critical Section 1 */ Some loads L1i interleaved with some stores S1j /* End Critical Section 1 */ R1: release(lock)</td><td>A2: acquire(lock) <br> /* Begin Critical Section 2 */ <br> Some loads L2i interleaved with some stores S2j <br> /* End Critical Section 2 */ R2: release(lock)</td><td>/* Arbitrary interleaving of L1i's & S1j's */ <br> /* Handoff from critical section 1 */ <br> /* To critical section 2 */ <br> /* Arbitrary interleaving of L2i's & S2j's */</td></tr></table>
...
$$
\text{ - S1, }\mathrm{S}2 \rightarrow  \mathrm{F}1 \rightarrow  \mathrm{S}3 \rightarrow  \mathrm{L}1\text{ loads }\mathrm{{SET}} \rightarrow  \mathrm{F}2 \rightarrow  \mathrm{L}2,\mathrm{L}3\text{ . }
$$
...
<table><tr><td>Core C1</td><td>Core C2</td><td>Comments</td></tr><tr><td>S1: data1 = NEW; <br> S2: data2 = NEW; <br> F1: FENCE <br> S3: flag = SET;</td><td>L1: r1 = flag; <br> B1: if (r1≠ SET) goto L1; <br> F2: FENCE <br> L2: r2 = data1; <br> L3: r3 = data2;</td><td>/* Initially, data1 & data2 = 0 & flag ≠ SET */ <br> /* L1 & B1 may repeat many times */</td></tr></table>
...
$$
\text{ - All L1i, All S1j } \rightarrow  \mathbf{{F13}} \rightarrow  \mathrm{{R11}} \rightarrow  \mathrm{{A21}} \rightarrow  \mathrm{{F22}} \rightarrow  \text{ All L2i, All S2j }
$$
...
<table><tr><td>Core C1</td><td>Core C2</td><td>Comments</td></tr><tr><td>F11: FENCE <br> A11: acquire(lock) <br> F12: FENCE <br> Some loads L1i interleaved with some stores S1j <br> F13: FENCE <br> R11: release(lock) <br> F14: FENCE</td><td>F21: FENCE <br> A21: acquire(lock) <br> F22: FENCE <br> Some loads L2i interleaved with some stores S2j <br> F23: FENCE <br> R22: release(lock) <br> F24: FENCE</td><td>/* Arbitrary interleaving of L1i's & S1j's */ <br> /* Handoff from critical section 1 */ <br> /* To critical section 2 */ <br> /* Arbitrary interleaving of L2i’s & S2j’s */</td></tr></table>
...
> 
1. 所有核心将其加载、存储和内存屏障 (FENCE) 插入到顺序 <m 中，遵守：
...
2. All cores insert their loads and stores to the same address into the order $< \mathrm{m}$ respecting:

> 
2. 所有核心将其对同一地址的加载 (load) 和存储 (store) 操作按顺序 $< \mathrm{m}$ 插入，并遵守：
...
> 
3. 每条加载 (load) 指令都从对同一地址的上一个存储 (store) 指令获取其值：
...
<table><tr><td colspan="6">Operation 2</td></tr><tr><td rowspan="5">Operation 1</td><td></td><td>Load</td><td>Store</td><td>RMW</td><td>FENCE</td></tr><tr><td>Load</td><td>A</td><td>A</td><td>A</td><td>X</td></tr><tr><td>Store</td><td>B</td><td>A</td><td>A</td><td>X</td></tr><tr><td>RMW</td><td>A</td><td>A</td><td>A</td><td>X</td></tr><tr><td>FENCE</td><td>X</td><td>X</td><td>X</td><td>X</td></tr></table>
...
![Figure 5.1: Two equivalent executions of Table 5.3's program.](U5_images/fig01.jpg)
...
![Figure 5.2: Two equivalent executions of Table 5.4's critical section program.](U5_images/fig02.jpg)
...
![Figure 5.3: Two XC implementations.](U5_images/fig03.jpg)
...
1. FENCEs can be implemented in several different ways (see Section 5.3.2), but they must enforce order. Specifically, regardless of address, the reorder unit may not reorder:

> 
1. FENCE 指令（FENCEs）可以通过多种不同方式实现（参见第 5.3.2 节），但必须强制顺序。具体而言，无论地址如何，重排序单元（reorder unit）不得对以下操作进行重排序：
...
2. For the same address, the reorder unit may not reorder:

> 
2. 对于相同的地址，重排序单元 (reorder unit) 可能不会进行重排序：
...
3. The reorder unit must ensure that loads immediately see updates due to their own stores.

> 
3. 重排序单元必须确保加载指令能够立即看到由自己发出的存储指令所产生的更新。
...
<table><tr><td>Code</td><td>TSO</td><td>XC</td></tr><tr><td>Acquire lock</td><td>RMW: test-and-set L / * read L, write L=1 */ if L==1, goto RMW /* if lock held, try again */</td><td>RMW: test-and-set L /* read L, write L=1 */ if L==1, goto RMW /* if lock held, try again */ FENCE</td></tr><tr><td>Critical Section</td><td>Loads and stores</td><td>Loads and stores</td></tr><tr><td>Release lock</td><td>Store L=0</td><td>FENCE <br> Store $\mathrm{L} = 0$</td></tr></table>
...
<table><tr><td>Core C1</td><td>Core C2</td><td>Comments</td></tr><tr><td>F11: FENCE <br> A11: acquire(lock) <br> F12: FENCE <br> S1: data1 = NEW; <br> S2: data2 = NEW; <br> F13: FENCE <br> R11: release(lock) <br> F14: FENCE</td><td>L1: r2 = data2; <br> L2: r1 = data1;</td><td>/* Initially, data 1 & data2 = 0 */ <br> /* Four Possible Outcomes under XC: (r1, r2) = <br> $\left( {0,0}\right) ,\left( {0,\text{ NEW }}\right) ,\left( {\text{ NEW },0}\right)$ , or (NEW, NEW) But has a Data Race */</td></tr></table>
...
<table><tr><td>Core C1</td><td>Core C2</td><td>Comments</td></tr><tr><td>F11: FENCE <br> A11: acquire(lock) <br> F12: FENCE <br> S1: data1 = NEW; <br> S2: data2 = NEW; <br> F13: FENCE <br> R11: release(lock) <br> F14: FENCE</td><td>F21: FENCE <br> A21: acquire(lock) <br> F22: FENCE <br> L1: r2 = data2; <br> L2: r1 = data1; <br> F23: FENCE <br> R22: release(lock) <br> F24: FENCE</td><td>/* Initially, data 1 & data2 = 0 */ <br> /* Two Possible Outcomes under XC: <br> (r1, r2) = <br> (0, 0) or (NEW, NEW) <br> Same as with SC */</td></tr></table>
...
![Figure 5.4: Optimal placement of FENCEs is undecidable.](U5_images/fig04.jpg)
...
<table><tr><td>Core C1</td><td>Core C2</td><td>Comments</td></tr><tr><td>S1: data1 = NEW;</td><td>L1: r1 = data1; <br> B1: if (r1 ≠ NEW) goto L1; <br> F1: FENCE <br> S2: data2 = NEW;</td><td>/* Initially, data1 & data2 = 0 */ <br> L2: r2 = data2; <br> B2: if (r2 ≠ NEW) goto L2; <br> F2: FENCE <br> L3: r3 = data1; /* r3==NEW? */</td></tr></table>
...
<table><tr><td>Core C1</td><td>Core C2</td><td>Core C3</td><td>Core C4</td></tr><tr><td>S1: data1 = NEW;</td><td>S2: data2 = NEW;</td><td>L1: r1 = data1; /* NEW */ F1: FENCE <br> L2: r2 = data2; /* NEW? */</td><td>/* Initially, data1=data2=0 */ <br> L3: r3 = data2; /* NEW */ F2: FENCE <br> L4: r4 = data1; /* NEW? */</td></tr></table>
...
<table><tr><td>Core C1</td><td>Core C2</td><td>Comments</td></tr><tr><td>S1: RELEASE-RC ${}_{SC}\mathrm{x} = \mathrm{{NEW}}$ ; <br> L1: ACQUIRE- ${\mathrm{{RC}}}_{SC}\mathrm{r}1 = \mathrm{y}$ ;</td><td>S2: y = NEW; FENCE RW, RW; L2: r2=x;</td><td>Initially x=y=0</td></tr></table>
...
<table><tr><td>Core C1</td><td>Core C2</td><td>Comments</td></tr><tr><td>S1: data2 = NEW; FENCE W, W</td><td>L1: r1 = pointer;</td><td>Initially pointer $= \&$ data1, data1 $= 0$</td></tr><tr><td>S2: pointer = &data2;</td><td>L2: r2=*r1;</td><td></td></tr></table>
...
<table><tr><td>Core C1</td><td>Core C2</td><td>Comments</td></tr><tr><td>L1: r1 = x;</td><td>L2: r2 = y;</td><td>Initially x=y=0</td></tr><tr><td>S1: y = r1;</td><td>S2: x = r2;</td><td></td></tr></table>
...
<table><tr><td>Core C1</td><td>Core C2</td><td>Comments</td></tr><tr><td>L1: r1=x; <br> B1: if r1 ≠ 42 return;</td><td>L2: r2 = y;</td><td>Initially $\mathrm{x} = \mathrm{y} = 0$</td></tr><tr><td>S1: y=42;</td><td>S2: x= r2;</td><td></td></tr></table>
...
<table><tr><td>Core C1</td><td>Core C2</td><td>Comments</td></tr><tr><td>S1: data1 = NEW; <br> S2: data2 = NEW; <br> F1: LWSYNC <br> S3: flag = SET;</td><td>L1: r1 = flag; <br> B1: if (r1 ≠ SET) goto L1; <br> F2: LWSYNC <br> L2: r2 = data1; <br> L3: r3 = data2;</td><td>/* Initially, data 1 & data2 = 0 & flag ≠ SET */ <br> /* Ensures S1 and S2 befor S3 */ <br> /* spin loop: L1 & B1 may repeat many times */ <br> /* Ensure B1 before L2 and L3 */</td></tr></table>
...
<table><tr><td>Core C1</td><td>Core C2</td><td>Comments</td></tr><tr><td>S1: x = NEW; <br> F1: HWSYNC <br> L1: r1 = y;</td><td>S2: y = NEW; <br> F2: HWSYNC <br> L2: r2 = x;</td><td>/* Initially, $\mathrm{x} = 0\& \mathrm{y} = 0$ */ <br> /* Ensure S1 before Li for i = 1,2 */</td></tr></table>
...
<table><tr><td>Core C1</td><td>Core C2</td><td>Core C3</td></tr><tr><td>S1: data1 = NEW;</td><td>L1: r1 = data1; <br> B1: if (r1 ≠ NEW) goto L1; <br> F1: LWSYNC <br> S2: data2 = NEW;</td><td>/* Initially, data 1 & data2 = 0 */ <br> L2: r2 = data2; <br> B2: if (r2 ≠ NEW) goto L2; <br> F2: LWSYNC <br> L3: r3 = data1; /* r3==NEW? */</td></tr></table>
...
<table><tr><td>Core C1</td><td>Core C2</td><td>Core C3</td><td>Core C4</td></tr><tr><td>S1: data1 = NEW;</td><td>S2: data2 = NEW;</td><td>L1: r1 = data1; /* NEW */ F1: HWSYNC <br> L2: r2 = data2; /* NEW? */</td><td>/* Initially, data1 = data2 = 0 */ <br> L3: r3 = data2; /* NEW */ F2: HWSYNC <br> L4: r4 = data1; /* NEW? */</td></tr></table>
...
![Figure 5.5: Comparing memory consistency models.](U5_images/fig05.jpg)
...
![Figure 5.6: (a) High-level language (HLL) and (b) hardware memory models.](U5_images/fig06.jpg)
...
1. allow all optimizations for high-performance DRF programs,

> 
1. 允许对高性能无数据竞争 (data-race-free, DRF) 程序的所有优化，




![Figure 5.7: Register allocation affects memory order.](U5_images/fig07.jpg)
...
2. unambiguously, specify the allowed behavior of all programs, and

> 
2. 明确地指定所有程序的允许行为（allowed behavior），并且




3. make this unambiguous specification simple.
