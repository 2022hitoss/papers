...
> 
摘要——对于 GPU 等高吞吐量处理器而言，一个关键组件是将大量核心与内存分区 (memory partition) 互连在一起的片上网络 (network-on-chip, NoC)。在这项工作中，我们从延迟 (latency) 和带宽 (bandwidth) 的角度，对跨越数代现代 NVIDIA GPU 的真实 GPU NoC 进行了详细分析。我们的分析识别出，基于核心和内存分区在 GPU 中的物理位置，核心与内存分区之间存在非均匀延迟 (non-uniform latency)。这种非均匀性可导致片上延迟 (on-chip latency) 出现高达约 \( {70}\% \) 的差异。相比之下，从核心到内存分区提供的带宽则大致均匀。然而，由多个 GPU“分区” (partition) 组成的近期 GPU 在分区之间通信时呈现出不同的片上延迟和带宽特性。基于我们对真实 GPU 互连 (interconnect) 的分析，我们讨论了潜在影响，包括其对侧信道攻击 (side-channel attack) 中所用时序 (timing) 的影响，以及对 NoC 微架构 (microarchitecture) 的影响。我们展示了如何利用这种非均匀延迟在 GPU 内实施时序侧信道攻击 (timing side-channel attack)，因为核心位置会影响性能（或时序）。此外，对 GPU NoC 的正确理解（以及正确假设）对于确保网络不会成为整体系统性能的瓶颈至关重要。
...
> 
索引术语——GPU (GPU)、片上网络 (network-on-chip)、时序侧信道攻击 (timing side-channel attack)
...
> 
并行应用的加速 (acceleration of parallel applications) 已成为现代计算 (modern computing) 的重要组成部分 [1]-[3]。这些新兴工作负载 (emerging workloads) 通常需要图形处理器 (graphics processing unit, GPU) 等高吞吐量架构 (high-throughput architecture)，并与 CUDA、OpenCL 等并行编程模型 (parallel programming model) 一起，使 GPU 能够广泛用于不同应用领域 (application domain) [4]。为维持高性能，GPU 需要支持高计算吞吐量 (compute throughput) 和高内存带宽 (memory bandwidth)。表 I 对本工作中使用的最近三代 NVIDIA GPU 作了高层次概述。核心 (core)（或流式多处理器 (streaming multiprocessors, SMs)）数量已经增加，以提供高计算吞吐量；内存容量和带宽 (memory capacity and bandwidth) 以及片上缓存 (on-chip cache) 也已显著增加。因此，互连大量片上组件的片上网络 (network-on-chip, NoC) [5] 正成为整体系统性能 (overall system performance) 的关键组成部分。
...
> 
表 I：NVIDIA GPU 的微架构 (microarchitecture) 比较。
...
> 
尽管已有大量关于 GPU 片上网络 (NoC) 及其对整体系统性能影响的研究，但大多数先前工作聚焦于基于仿真的评估，未必能反映真实 GPU 中的 NoC。据我们所知，这是首批对真实、现代 GPU NoC 进行详细分析的工作之一。在这项工作中，我们首先从延迟 (latency) 和吞吐量 (throughput)（带宽 (bandwidth)）两个方面对现代 GPU NoC 进行详细分析。我们对 NVIDIA GPU 的分析表明，“平均而言”，所有核心 (core) 之间的片上延迟相似；然而，GPU NoC 中存在到内存分区 (memory partition) 的非均匀延迟，这取决于核心与 L2 切片 (L2 slice) 的物理布局。相比之下，到远程 L2 的带宽（或 L2 互连结构带宽 (L2 fabric bandwidth)）在大体上是均匀的，无论核心与内存分区如何布局。最近的 GPU（例如 A100、H100）具有多个 GPU“分区”(partition)，当流量从一个分区跨越到另一个分区时，会在延迟和带宽两方面引入不同特性。
...
> 
基于该分析，我们探讨其对 GPU 架构两个方面的影响——时序侧信道攻击 (timing side-channel attacks) 和 GPU NoC 微架构 (GPU NoC microarchitecture)。最近已有关于 GPU 中时序侧信道攻击的工作，这些工作基于各种微架构特征 [6]-[11]。然而，时序攻击 (timing attacks) 从根本上依赖于测量执行时间 (execution time)（或延迟 (latency)）。我们展示 GPU NoC 的非均匀延迟 (non-uniform latency) 如何能在时序侧信道攻击中被利用，从而可能促成攻击，也有助于缓解侧信道攻击。此外，与传统的多核 NoC (multicore NoC) 相比，GPU NoC 架构面临独特挑战，因为将各组件互连在一起需要高带宽 (high-bandwidth)（以及高吞吐量 (high-throughput)）。基于对真实 GPU 的分析，我们强调未来 GPU NoC 设计中的一些挑战（和机遇），以及在探索 GPU NoC 架构时的陷阱。具体而言，这项工作的主要贡献包括以下内容。
...
> 
- 我们提供了对跨越数代 NVIDIA 图形处理器 (GPU) 的真实片上网络 (network-on-chip, NoC) 的详细表征与分析。一个关键观察是，延迟 (latency) 可能是不均匀的，但带宽 (bandwidth) 在核心 (core) 与内存分区 (memory partition) 之间是均匀提供的。
...
> 
- 基于片上网络 (NoC) 表征，我们展示了其对时序侧信道攻击 (timing side-channel attack) 的影响，以及非均匀延迟 (non-uniform latency) 如何既能在攻击期间被利用，又能作为针对时序侧信道攻击的防御被利用。
...
> 
- 我们还展示了在GPU架构探索中NoC假设的影响，以确保片上网络 (on-chip network)（或片上带宽 (on-chip bandwidth)）不会成为整体系统性能的瓶颈。
...
> 
图形处理器 (GPU) 是一种高度并行的处理器，可以同时执行数千个线程，从而实现高吞吐量处理 [12]。一个 GPU 由多个核心或流式多处理器 (Streaming Multiprocessor, SM) 以及内存分区 (memory partition, MP) 组成，它们通过片上网络 (network-on-chip, NoC) 互连在一起。以 CUDA/OpenCL 等 GPU 专用编程模型编写的 GPU 上下文 (context) 包含多个线程块 (thread block)，并且线程块调度器 (thread block scheduler) [13] 会调度每个线程块。在 GPU 流水线中，线程束 (warp) 或一组线程是执行单元，并针对每个线程以不同数据执行相同指令。此外，多个线程束基于线程束调度算法 (warp-scheduling algorithm) [14]- [17] 并发执行。每个核心都有私有的一级缓存 (L1 cache) 和共享内存 (shared memory)，而全局共享的二级缓存 (L2 cache) 靠近内存控制器 (memory controller, MC)。每个内存分区 (MP) 包含多个二级缓存切片 (L2 slice) 和一个管理片外内存 (off-chip memory) 的内存控制器 (MC) [18], [19]。NoC 将 SM 与内存分区 (MP) 互连。GPU 架构是分层的：在一个纹理处理簇 (Texture Processing Cluster, TPC) 内，两个 SM 通过多路复用器 (multiplexer) 连接；7 到 9 个 TPC 组成一个图形处理簇 (Graphics Processing Cluster, GPC)，6 到 8 个 GPC 组成一个 GPU [12], [20], [21]。近期的 GPU（例如 A100、H100）包含一个中央互连 (central interconnect)，其将 GPU 分为左、右两个“分区” (partitions)。在这项工作中，我们分析 GPU 互连，并探讨 NoC 对高吞吐量处理器的影响，包括延迟 (latency) 和带宽 (bandwidth) 两个方面。
...
> 
对近期 GPU 系统（包括 V100 [22] 和 A100 [23]）的分析已经完成，并且一些工作分析了 GPU 中的内存层次结构 (memory hierarchy) [24]。尽管这些工作提供了对 GPU 各个组件的深入理解，但它们没有提供对 GPU 内部互连 (interconnect) 的详细分析。多 GPU 系统 (multi-GPU systems) 也已被分析。
...
> 
M[] : L2 切片 (L2 slice) 到数据 \( \left( {D\left\lbrack  \right\rbrack  }\right) \) 的索引映射 (index mapping)
...
> 
输出 L[][]：测量的 L2 访问延迟 (Measured L2 access latency)
...
> 
for \( n \leftarrow  1 \) to \( N \) do \( \vartriangleright  N \) : # 核心数 (number of cores)
...
> 
以理解 GPU 间互连 (GPU-to-GPU interconnect) [25]-[27] 的影响，但他们对片上互连 (on-chip interconnect) 的讨论有限。
...
> 
面向高吞吐量处理器的片上网络 (Network-on-chip, NoC) 与多核 CPU 的 NoC 相比，面临着不同的挑战。已有工作提出了面向通用图形处理器 (GPGPU) 的吞吐量高效 NoC [28]，其中基于多对少对多 (many-to-few-to-many) 流量模式识别出“应答 (reply)”网络瓶颈。为解决该瓶颈，已有工作提出了替代 NoC 架构 [29]-[32]，以尽量减小应答 (reply) 网络的影响。为减少片上流量，已提出合并 (coalescing)，以减少注入 NoC 的片上流量 [33]。然而，许多这类工作通常假设诸如二维网格 (2D-mesh) 拓扑等多跳 (multi-hop) 网络，并可能导致片上带宽瓶颈。
...
> 
近期工作 [34], [35] 假定了一种分层交叉开关 (hierarchical crossbar) 组织方式。据我们所知，真实系统中的图形处理器 (GPU) 互连与分层交叉开关的组织方式类似；然而，这些工作并未对真实 GPU 片上网络 (NoC) 进行详细分析。更重要的是，许多先前工作没有考虑 NoC 带宽对 L2 带宽 (L2 bandwidth) 的影响——即，需要对 NoC 带宽进行配置，以能够提供足够的 L2 带宽。Accel-sim [36] 还论证了 L2 带宽如何可能成为一个瓶颈，阻碍其模拟器在现代 GPU 中进行更准确的建模。
...
> 
在本工作中，我们使用了前文所述的三款图形处理器 (GPU)（表 I）。为了分析 GPU 片上网络 (NoC) 的性能影响，我们编写了合成微基准测试内核，以隔离 GPU 内不同因素和组件的影响。特别地，这些测量侧重于理解 NoC 的延迟 (latency) 和带宽 (bandwidth)。
...
> 
1) 延迟测量 (Latency Measurement)：本工作中如何测量延迟的高层概述如算法1 (Algorithm 1)所示。仅使用一个线程束 (warp) 内的一个线程 (thread) 发送请求 (request)，以最小化 GPU 中其他组件的影响——因此，不存在合并 (coalescing)，也不会对 GPU 内任何资源产生争用 (contention)。所进行的内存访问 (memory access) 保证绕过 L1，并且除非另有说明，工作集 (working set) 适合放入 L2。对 \( D\left\lbrack  \right\rbrack \) 的内存访问用于预热 (warm up) L2缓存 (L2 cache)，以确保所有访问都在 L2 中命中 (hit)。测得的（往返）延迟 (round-trip latency) 包括核心 (core)（SM）、NoC 和 L2访问延迟 (L2 access latency)。由于测量是在不同 SM 和 L2切片 (L2 slice) 上进行的，核心或 L2 的延迟大致相同，但总体延迟 (overall latency) 的变化来自 NoC 的影响。评估 (evaluation) 中使用的内存访问通过 -dlcm=cg 编译选项 (compilation option) 绕过每个SM的L1缓存 (per-SM L1 cache)。延迟使用每个 SM 内提供的硬件时钟计数器 (hardware clock counter) clock () 来测量。
...
> 
算法 2 带宽测量算法 (Bandwidth measurement algorithm)。
...
> 
M[s][]：映射到 L2 切片 (L2 slice) \( s \) 的数据 \( \left( {D\left\lbrack  \right\rbrack  }\right) \) 索引的集合
...
> 
function L2_BANDWIDTH \( \left( s\right) \; \vartriangleright  s \) : 目标 L2 切片 (target L2 slice)
...
> 
图 1：(a) 在 V100 图形处理单元 (GPU) 上，从一个流式多处理器 (SM)（即 SM 24）到所有 L2 切片 (L2 slice) 的非均匀 L2 访问延迟 (L2 access latency)，以及 (b) 每个图形处理集群 (GPC) 内的平均延迟 (average latency) 和延迟变化 (latency variation)。
...
> 
在本节中，我们提供对 GPU 的片上 (on-chip) 网络 (network-on-chip, NoC) 延迟 (latency) 的分析。我们展示 GPU 中如何观察到非均匀 (nonuniform) 延迟，这通常由核心 (core) 和内存分区 (memory partition, MP)（以及 L2 切片 (L2 slice)）的物理位置决定。此外，GPU 规模的不断增大导致出现由多个“分区” (partition) 组成的单体式 (monolithic) GPU，我们分析此类 GPU 分区对片上延迟的影响。
...
> 
从 SM 到 L2 slice 的 L2 访问延迟或往返延迟——包括核心 (core, SM) 延迟、NoC 延迟和 L2 访问延迟——如图 1 所示。L2 访问延迟通过一次在本地 L1 缓存中缺失、在共享 L2 缓存中命中的内存读取访问来测量。图 1(a) 绘制了 V100 GPU 上从某一特定核心 (SM 24) 到 32 个不同 L2 slice 的 L2 访问延迟分布，\( x \) 轴是 NVIDIA 性能分析器 [37] 提供的 L2 slice ID。尽管该图仅展示了一个 SM，其他 SM 也具有非均匀的延迟分布，最低延迟为 175 个周期，而最高延迟为 248 个周期，平均延迟约为 212 个周期。对不同 L2 slice 的访问共享相同的组成部分（例如核心延迟、L2 延迟），但主要差异在于 L2 slice（以及 SM 位置）的物理位置。
...
> 
观察 #1：从核心 (core)（SM）经由 GPU 片上网络 (NoC) 到各个 L2 切片 (L2 slice) 的延迟 (latency) 是非均匀 (nonuniform) 的。
...
> 
\( {}^{1} \) 用于 V100 的性能分析器 (profiler)（以及性能计数器 (performance counters)）在 A100 或 H100 上已不再受支持 [37]。因此，采用了更手动的方法：编写了两个内核 (kernel)，其中一个内核始终访问单个地址（单个切片 (single slice)），而另一个内核所访问的地址则不断变化。当出现“争用 (contention)”时，我们假设正在访问的是同一个 L2 切片 (L2 slice)。
...
> 
\( {}^{2} \) 所测得的带宽 (measured bandwidth) 不一定就是 L2 带宽 (L2 bandwidth)，因为 L2 带宽 (L2 bandwidth) 表示缓存读写性能 (cache read/write performance)。相比之下，我们测量的是到/从 L2 的互连 (interconnect) 或互连结构 (fabric) 带宽 (bandwidth)。
...
> 
观察 #2：来自一个图形处理集群 (GPC) 的平均 L2 延迟 (L2 latency)（即，从某个 GPC 内的流式多处理器 (SM) 到所有 L2 切片 (L2 slice) 的延迟）在所有 GPC 之间相似；然而，L2 延迟变化存在于一个 GPC 内的 SM 之间，并且延迟变化在不同 GPC 之间有所不同。
...
> 
如第 II 节 (Sec II) 前面所讨论的，图形处理器 (GPU) 按层次组织：多个核心 (core) 位于一个图形处理集群 (GPC) 内，而多个 L2 切片 (L2 slice) 位于单个内存分区 (MP) 内。L2 访问延迟 (L2 access latency)（以及片上网络 (network-on-chip, NoC)）的确切延迟由核心 (core)（或流式多处理器 (SM)）在 GPC 内的物理位置以及 L2 切片在 MP 内的物理位置决定——此外还取决于 GPC 和 MP 本身的位置。为了更好地理解放置 (placement) 的影响，我们在图 3 (Figure 3) 中绘制了 L2 延迟，但首先根据其 MP ID 将 L2 切片 ID 分组在一起——即位于同一 MP 内的 L2 切片被分为一组。然后根据延迟值对 L2 切片 ID 排序——从最低 L2 延迟到最高。不同 SM 之间切片 ID 的排序顺序相同——例如，对于 MP 0，所有四个图都按以下 L2 切片 ID 顺序显示 L2 延迟：0、4、27、31、... 出于说明目的，仅显示了四个 SM 的延迟分布，其中一组两个 SM 来自同一 GPC。尽管确切延迟值不同，来自同一 GPC 的 SM 在延迟变化方面显示出相同趋势（例如，图 3(a, b) 和图 3(c, d)）。尽管不同 MP 之间的延迟不同，但每个 MP 内的延迟趋势在不同 SM 之间也相似。
...
> 
基于裸片照片 (die photo) [38] 和我们的分析，V100 的近似逻辑平面布局图 (logical floorplan) 如图 4 所示。该图包含一个图形处理集群 (GPC)（即 GPC 4）的 SM (流多处理器, streaming multiprocessor) 近似位置，以及一个内存分区 (memory partition, MP)（即 MP 3）的 L2 切片 (L2 slice) 位置。实际的 SM ID 或 L2 切片 ID 并不重要，但该图展示了核心 (core)（SM）和 L2 切片的相关位置。位置靠近的 SM 和 L2 切片具有最低的延迟 (latency)（例如，L2 切片 17 和 SM64 的 L2 延迟为 180 个周期 (cycle)），而距离最远的（例如，L2 切片 15 和 SM4）具有最高延迟，为 217 个周期。为简单起见，该图仅显示一个 GPC 和一个 MP，但这一趋势在不同 GPC 和 MP 组合中均成立。详细的延迟分析跨越
...
> 
图 2：两个不同图形处理集群 (GPC) 的 L2 延迟 (L2 latency) 直方图分布，(a) GPC0 和 (b) GPC2。这两个 GPC 的平均 L2 延迟非常相似，但延迟分布不同。
...
> 
图 3：多个流式多处理器 (SM) 的延迟变化图 (latency variation plot)，包括来自图形处理集群 (GPC) 0 的两个 SM（(a) SM60 和 (b) SM24）以及来自图形处理集群 (GPC) 4 的两个 SM（(c) SM64 和 (d) SM28）。L2 切片 (L2 slice) 索引经过排序，使得同一内存分区 (memory partition) 中的 L2 切片被分组在一起。
...
> 
不同流式多处理器 (SM) 与 L2 切片 (L2 slice) 如图 5 所示。结果表明，不同的 SM 位置会导致 L2 延迟 (L2 latency) 产生恒定差异，而某些 L2 切片相比其他 L2 切片始终具有更低的延迟。
...
> 
观察 #3：非均匀的 L2 延迟 (latency) 由流式多处理器 (SM) 在图形处理集群 (GPC) 内的物理位置以及 L2 切片 (L2 slice) 在内存分区 (memory partition) 内的物理位置决定。
...
> 
皮尔逊相关 (Pearson Correlation) [39] 是一种常用的衡量两个数据集之间线性相关 (linear correlation) 的度量，可用于衡量两个 SM 之间 L2 延迟 (latency) 分布的相似性。该相关性 (correlation) \( \left( r\right) \) 定义如下，
...
> 
其中 \( n \) 是样本数量，而 \( {X}_{i},{Y}_{i} \) 是来自两个不同 SM 的各个延迟样本 (latency samples)。皮尔逊相关 (Pearson correlation) 为 0 表示无相关性，为 1 表示完全正（线性）相关，为 -1 表示完全负相关。图 3(a, b) 中的延迟分布 (latency distribution) 的皮尔逊相关为 0.998，表明分布几乎相同。然而，图 3(a, c) 之间的相关性约为 -0.365，表明相关性极小。
...
> 
V100 的皮尔逊相关 (Pearson correlation) 热力图 (heatmap) 如图 6(a) 所示。每个轴表示不同的 SM，并依据其 GPC ID 分组。该热力图显示，GPC0 中的 SM 与 GPC0 和 GPC1 中的每个其他 SM 之间均具有高相关性。总体而言，同一 GPC 内的 SM 显示出（近乎）完美相关，而两个相邻 GPC 构成的组具有非常高的正相关值。然而，距离更远的 GPC 之间相关性更低。分析表明，与其他两个 GPC（GPC2&3）相比，两组 GPC（GPC0&1 和 GPC4&5）具有相似特征。这可能源于 GPU 的对称物理布局：GPC2&3 位于中心，而 GPC0&1 和 GPC4&5 位于边缘（图 4）。
...
> 
图 4：V100 GPU 的框图（近似逻辑布局图），展示了 L2 slice 在 MP 内的位置以及 SM 在 GPC 内的位置。
...
> 
图 5：V100 上 GPC4 中的 SM 到内存分区 (MP) 3 内不同 L2 切片 (L2 slice) 的 L2 访问延迟 (L2 access latency)。物理上更接近的 SM 和 L2 切片具有更低的延迟。
...
> 
观察结果 #4：使用皮尔逊相关 (Pearson correlation) 得出的片上延迟分布 (on-chip latency distribution) 相似性，可用于确定 SM 的放置位置。
...
> 
A100 的皮尔逊相关系数 (Pearson correlation) 热力图（图 6(b)）显示，在单个图形处理集群 (graphics processing cluster, GPC) 内，各流多处理器 (streaming multiprocessor, SM) 之间存在一定差异；这与 V100 不同，后者的 GPC 内所有 SM 之间的皮尔逊相关系数相对恒定。由于从 V100 到 A100，GPU 的规模——无论物理裸片尺寸还是每个 GPC 的 SM 数量——都有所增大，GPU 实际上由两个“分区” (partition) 组成，并影响了延迟 (latency) 特性。虽然未展示，但在 V100 上观察到的非均匀二级缓存 (L2) 延迟也存在于 A100 和 H100 中。然而，对于 A100，不同 GPC 之间的 SM 不一定具有相同的延迟分布 (latency distribution) 特性（如在 V100 中观察到的那样）。同一 GPC 内的 SM 之间的延迟几乎相同，如热力图中的对角线所示，但 V100 中与相邻 GPC 存在的相似性在 A100 中减弱了。\( {}^{3} \) 如图 6(c) 所示，H100 中一个 GPC 内 SM 之间的差异要大得多。一组 4 个或 6 个 SM（即 2 个或 3 个纹理处理集群 (texture processing cluster, TPC)）具有相似的延迟特性，而这些特性与同一 GPC 内其他 SM 组相比非常不同。这种延迟分布上的相似性表明，在 TPC 和 GPC 层级之间可能存在一个新的核心层级，我们将其称为计算处理集群 (compute processing cluster, CPC)。\( {}^{4} \)
...
> 
图 6：在 (a) V100、(b) A100 和 (c) H100 GPU 上，L2 延迟 (latency) 的皮尔逊相关系数 (Pearson correlation) 热力图 (heatmap)。对于 A100 和 H100，GPC 0-3 位于左侧“分区 (partition)”，而 GPC 4-7 位于右侧“分区 (partition)”。
...
> 
图 7：(a) H100 中一个 GPC 内部提供 SM 到 SM 通信的 CPC 层次结构 (CPC hierarchy) 框图，以及 (b) 基于 CPC 位置的平均 SM 到 SM 延迟 (latency)。
...
> 
图 7(a) 展示了 GPC 内 CPC 层级 (CPC hierarchy) 的高层框图 (high-level block diagram)：三个 CPC 位于一个 GPC 内，并通过 SM 到 SM 网络 (SM-to-SM network) 互连。与前几代 GPU 相比，H100 的一个独特特性是分布式共享内存 (distributed shared memory) [12]，它允许通过 SM 到 SM 网络访问远程共享内存 (remote shared memory)。为了理解 SM 到 SM 网络的延迟 (latency) 特性，使用了一个合成内核 (synthetic kernel)，类似于算法 1，但关键区别在于，加载 (load) 被发往同一 GPC 内的远程共享内存。GPC 内不同 CPC 组合之间的 SM 到 SM 通信的平均延迟如图 7(b) 所示，并以 (src, dest) CPC 配对信息表示。CPC 的相对距离会影响延迟——例如，当 CPC0 内的两个 SM 通信时延迟最低（196 个周期），但对于 CPC2 中 SM 之间的通信，延迟增加到约 213 个周期，而 CPC2 很可能距离最远。另一个
...
> 
\( {}^{3} \) 其原因尚不明确，但我们怀疑更多的 SM 数量以及更大的芯片面积对延迟相关性有一定影响。
...
> 
\( {}^{4} \) 尽管 NVIDIA 文档并未披露这一层级结构 (hierarchy)，一些硬件拆解 (hardware teardown) [40] 也识别出了一种潜在的中间层级结构，他们将其称为 “CPC” 或计算处理集群 (Compute Processing Cluster)。在本工作中，我们沿用相同的术语。这种层级结构的存在尚未得到确认；然而，我们的工作展示了这种层级结构对互连性能 (interconnect performance) 的影响。
...
> 
观察 #5：更大规模的 GPU (larger GPUs)（具有多个“分区” (partition)）会导致不同的延迟特性 (latency characteristics) 和更强的非均匀性 (non-uniformity)。具体而言，H100 在 TPC 与 GPC 之间还包含另一级 SM 核心层次结构 (SM core hierarchy)，这会影响延迟特性 (latency characteristics)。
...
> 
最近的 GPU (A100、H100) 在跨越 GPU“分区 (partition)”时，其 L2 延迟 (latency) 也（与 V100 相比）呈现出显著不同的特征。在图 8(a, b, c) 中，我们绘制了这 3 个不同 GPU 上的平均 L2 访问延迟 (latency)——该延迟是对来自一个图形处理集群 (GPC) 内所有流多处理器 (SM) 到单个内存分区 (MP) 内不同 L2 切片 (L2 slice) 的请求取平均得到的。V100 的平均延迟约为 212 个周期（图 8(a)）（与本节早先的结果类似）；然而，对于 A100，平均 L2 延迟差异显著——例如，来自 GPC 1、2、6、7 的请求的延迟与 V100 类似，但来自 GPC 0、3、4、5 的 L2 访问具有高得多的延迟。在我们的评估中，我们假设目的端（或 L2 切片）位于左侧“分区”。由于 GPC 1、2、6、7 与目的端 L2 切片位于同一分区，因此延迟较低，因为请求的目的端位于近端分区。然而，对于 GPC 0、3、4、5，目的端 L2 切片位于远端分区——导致约 400 个周期的高得多延迟——这是由往返跨越 GPU 分区所产生的额外延迟造成的。
...
> 
观察 #6：具有多个 GPU“分区 (partition)”的最新 GPU 会根据 L2 缓存策略 (L2 cache policy) 具有额外的均匀性 (uniformity)。
...
> 
图 8：在 (a, d) V100、(b, e) A100 和 (c, f) H100 上，从一个图形处理集群 (GPC) 到一个内存分区 (MP) 的平均 L2 命中延迟 (L2 hit latency)（上），以及从全局内存 (global memory) 到 L2 的平均 L2 未命中代价或延迟 (L2 miss penalty or latency)（下）。
...
> 
图 9：(a) 片上聚合 L2 和全局内存 (global memory) 带宽。当在 V100 中由 (b) 单个流式多处理器 (SM) 和 (c) 单个图形处理集群 (GPC) 访问时，到单个 L2 切片 (L2 slice) 的 L2 带宽分布。
...
> 
在本节中，我们分析图形处理器 (GPU) 中的片上带宽 (on-chip bandwidth)，特别是到 L2 切片 (L2 slice)（或内存分区 (memory partition)）的带宽，或者 L2 互连结构带宽 (L2 fabric bandwidth)。
...
> 
在图 9(a) 中，我们比较了在不同图形处理器 (GPU) 上测得的总聚合片上 L2 互连结构带宽 (L2 fabric bandwidth) 和全局内存带宽 (global memory bandwidth)。聚合 L2 互连结构带宽让所有流式多处理器 (SM) 将流量发送到所有 L2 切片 (L2 slice)（并在 L2 中命中），而内存带宽 (memory bandwidth) 测量则假设访问不会在 L2 中命中。两种带宽均使用顺序内存访问进行测量，且内存带宽达到峰值最大内存带宽的约 85-90%。尽管片外内存带宽 (off-chip memory bandwidth) 持续增长，片上 L2 带宽 (on-chip L2 bandwidth) 也相应增长，并且比片外内存带宽高约 \( {2.4} \times \) 到 \( {3.5} \times \)。虽然具有更高的 L2 带宽并不一定令人惊讶 [22]，但它确实对 GPU 片上网络 (NoC) 的设计具有影响，正如我们在第 VI 节中所讨论的，因为该 NoC 会显著影响 L2 互连结构带宽。
...
> 
观察 #7：在现代图形处理器 (GPU) 中，片上聚合（总）L2 互连结构带宽 (L2 fabric bandwidth) 超过片外内存带宽 (off-chip memory bandwidth) 的总量。
...
> 
尽管片上延迟 (on-chip latency) 是非均匀的（观察 3 (Observation 3)），但到不同目的端 (destination)（或 L2 切片 (L2 slice)）的带宽 (bandwidth) 更为均匀。图 9(b) 展示了 V100 中当一个 SM 向单个 L2 切片发送流量 (traffic) 时的 L2 带宽直方图，显示了在所有源 (source)（SM）与目的端 (destination)（L2 切片）组合上的带宽分布。从单个 SM 到单个 L2 切片的平均带宽约为 34 GB/s，其中 \( \sigma  = {0.147}\mathrm{{GB}}/\mathrm{s} \) 。当一个 GPC 内的所有 SM 都向单个 L2 切片发送流量时，可达到的带宽约为 \( {85}\mathrm{{GB}}/\mathrm{s} \)，其中 \( \sigma  = {0.06}\mathrm{{GB}}/\mathrm{s} \)，如图 9(c) 所示——这是对所有 GPC 与一个目的 L2 切片组合测量得到的。这两个结果都表明带宽分布很集中，并表明从不同核心 (core)（SM）和 GPC 提供给每个 L2 切片的带宽大致相似。结果还表明，要使单个 L2 切片带宽饱和，至少需要 4 个 SM。
...
> 
\( {}^{5} \) 这些结果是分别针对每个图形处理集群 (GPC) 收集的，但由于 H100 中的本地缓存 (local caching)，同一地址的 L2 切片 (L2 slice) ID 可能在不同 GPC 之间变化。然而，为简单起见，\( x \) 轴只显示 8 个不同的 L2 切片，且不同 GPC 不一定访问同一组 L2 切片。
...
> 
图 10：不同 GPU 架构间的互连输入加速比 (input speedup) 比较。
...
> 
图 11：展示 GPU NoC 内所提供的各种互连加速比 (interconnect speedup) 的框图。
...
> 
观察 #8：虽然对不同 L2 切片 (L2 slice) 的延迟 (latency) 是非均匀的，但对这些不同 L2 切片的带宽 (bandwidth) 是（大体上）均匀的。
...
> 
在这项工作中，输入加速比 (input speedup) 被定义为提供到 GPU 互连 (GPU interconnect) 中的额外带宽 [5]。GPU 以分层方式组织，输入加速比对于最小化由分层带宽共享 (hierarchical bandwidth sharing) 导致的潜在性能瓶颈是必要的。在我们评估的 GPU 中，两个流式多处理器 (SM) 聚集在一个纹理处理集群 (TPC) 中，而 TPC 加速比 (TPC speedup)（图 11）实际上是 TPC 输出与 SM 输出的带宽比。对于 2 比 1 的共享，TPC 加速比为 1 意味着跨两个 SM 只能提供 50% 的带宽。TPC 加速比的值为 2 会带来从两个 SM 进入片上网络 (NoC)、朝向目的 L2 切片 (L2 slice) 的“完整”带宽。加速比被测量为 \( x \) 个 SM 相对于一个 SM 的带宽（性能）比，其中这些 SM 向所有 L2 切片发送流量。对于 TPC 加速比，\( x = 2 \)，而对于 \( {\mathrm{{GPC}}}_{l}, x \) 是 TPC 的数量，对于 \( {\mathrm{{GPC}}}_{g}, x \) 是 TPC 的数量乘以每个 TPC 的 SM 数量。6
...
> 
图 12：(a) SM0 和 (b) SM2 在 A100 上的 L2 切片带宽 (L2 slice bandwidth)，其中 SM0 和 SM2 位于不同的 GPU 分区 (GPU partition)。
...
> 
不同 GPU 之间的加速比 (speedup) 在图 10 中针对读 (Reads) 和写 (Writes) 给出。对于读 (Reads)，从流式多处理器 (SM) 所需的带宽（即小的请求消息 (request messages)）低于到 SM 的带宽（即带有缓存行数据 (cache line data) 的大回复消息 (reply message)），而写 (Writes) 则相反。对于纹理处理簇 (TPC) 加速比，所有 GPU 的读 (Reads) 都提供全带宽（即 TPC 加速比为 2）；然而，对于写 (Writes)，在 V100 上仅实现了 \( {1.09} \times \) 的加速比。对于 A100 和 H100，TPC 写 (Writes) 也实现了全带宽。对于 \( {\mathrm{{GPC}}}_{l} \)，实现全带宽分别需要在 V100、A100 和 H100 上达到 7、8 和 9 的加速比。V100 达到该加速比的约 50%，而 H100 接近 85%，加速比接近 8。评估表明，提供输入带宽加速比 (input bandwidth speedup) 是为了最小化分层组织 (hierarchical organization) 带来的任何性能影响，并且较新的 GPU 具有更多的片上带宽 (on-chip bandwidth) 或输入加速比 (input speedup)。特别地，与 \( {\mathrm{{GPC}}}_{l} \) 相比，\( {\mathrm{{GPC}}}_{g} \) 提供了额外的加速比，尤其是在 A100 和 H100 上。对于 H100，还展示了计算处理簇 (CPC) 加速比，结果表明新增的层次结构对读访问 (Read accesses) 没有任何影响，但当 CPC 内的所有 SM 都用于写 (Writes) 时，会出现轻微的性能下降，因为仅提供了约 4.6 的加速比（相比之下，实现全带宽需要 6 的加速比）。
...
> 
观察 #9：尽管 GPU 采用分层组织，但输入加速 (input speedup) 的存在是为了向片上互连 (on-chip interconnect) 提供更高的带宽。
...
> 
在具有多个 GPU 分区 (partition) 的近期 GPU（即 A100、H100）中，跨分区时会引入额外的非均匀延迟 (non-uniform latency)（Observation 6），但在本节中，我们展示访问不同分区中的 L2 切片 (L2 slice) 时如何出现带宽非均匀性 (bandwidth non-uniformity)。A100 的 L2 带宽非均匀性示例如图 12 所示。SM0 与 L2 切片 0-39 位于同一分区，而 SM2 与 L2 切片 40-79 位于同一分区。对于“近端”分区 (near partition)，可实现更高带宽（约 39.5 GB/s），但对于到“远端”分区 (far partition) 的 L2 访问，带宽降至约 26 GB/s。对于位于另一分区的 SM2，可实现相似的 L2 带宽，但“近端”和“远端” L2 切片互换。
...
> 
\( {}^{6}{\mathrm{{GPC}}}_{l} \) 是纹理处理集群 (texture processing cluster, TPC) 与图形处理集群 (graphics processing cluster, GPC) 层级之间的局部 GPC 加速比 (speedup)，而 \( {\mathrm{{GPC}}}_{g} \) 是当 GPC 内所有流式多处理器 (streaming multiprocessor, SM) 都在发送流量时的全局加速比。
...
> 
图 13：在 (a) A100 和 (b) H100 GPU 中，到单个 L2 切片 (L2 slice) 的 L2 带宽 (bandwidth) 分布。
...
> 
图14：随着发送流量的SM数量增加，A100上单个L2切片带宽的变化。
...
> 
图 13(a) 展示了 A100 中每个 SM 访问单个 L2 切片 (L2 slice) 时的带宽分布 (bandwidth distribution)。其他 L2 切片也显示出类似的双峰分布 (bimodal distribution)——较高峰值出现在与该 L2 切片处于同一分区 (partition) 的 SM 上，而较小峰值出现在远程分区 (remote partition) 的 SM 上。图 13(b) 展示了 H100 的 L2 带宽分布，由于本地化 L2 缓存 (localized L2 cache) [12]，其表现出单峰。与 V100 相比，A100 和 H100 均具有更高的每 L2 切片带宽 (per-L2 slice bandwidth)，但变化也更大。
...
> 
为了理解在 A100 中访问远程分区 (remote partition) 的二级缓存 (L2) 时带宽 (bandwidth) 较低的原因，图 14 展示了随着更多流式多处理器 (SM) 向同一个 L2 切片 (L2 slice) 发送流量，L2 切片带宽如何增加。在大约 8 个 SM 时，无论 L2 位于近端分区 (near partition) 还是远端分区 (far partition)，带宽都会饱和。然而，对于较少数量的 SM，远端分区能够达到的带宽更低——即，对于一个或两个 SM，远端分区可达到的带宽最多低 \( {28}\% \)。由于到远端分区 L2 切片的延迟 (latency) 更高，根据利特尔定律 (Littel's Law)，在足够多的 SM 用于使带宽饱和之前，较长的延迟会导致较低的带宽。
...
> 
观察 #10：与以往的 GPU 相比，近期的 GPU（例如 A100、H100）具有更大的片上带宽 (on-chip bandwidth)（以及更大的加速比 (speedup)）；然而，这也会导致非均匀带宽 (nonuniform bandwidth)。
...
> 
由于单个内存分区 (memory partition, MP) 内存在多个 L2 切片 (L2 slice)，进入该 MP 的带宽 (bandwidth) 可能成为瓶颈，因此需要来自片上网络 (NoC) 输出的 L2 输入加速比 (L2 input speedup)（图 11）。为了评估 L2 输入加速比，我们改变 L2 切片数量（及其在各 MP 间的分配），并评估一个所有流式多处理器 (SM) 均发送流量的内核。连续 MP (Contiguous MP) 中使用的 L2 切片位于同一 MP 内，而分布式 MP (Distributed MP) 中的 L2 切片分散在不同 MP 上——因此，对于连续 MP，所访问的 MP 数量不变，但对于分布式 MP，MP 数量从 1 增加到 4。结果表明，在 V100 上，两种方法之间的带宽（性能）差异极小（图 15(a)）——这表明 L2 输入加速比接近理想值，从而最小化 L2 切片共享 NoC 输出带宽的影响。
...
> 
图 15：V100 上的 L2 带宽，其中 (a) 所有流式多处理器 (SM) 访问不同数量的 L2 切片 (L2 slice)，(b) 不同数量的 SM 访问一个内存分区 (MP)，以及 (c) 14 个 SM 访问不同数量的 MP。
...
> 
图 16：在 NVIDIA V100 GPU 上测量的 (a) BFS 和 (b) 高斯 (Gaussian) 工作负载的内存访问模式。
...
> 
相比之下，在向一个内存分区 (memory partition, MP) 发送流量时改变流式多处理器 (streaming multiprocessor, SM) 位置会导致显著的性能下降（图 15(b)）。连续 SM (contiguous SM) 将所有 SM 放在 1 个图形处理集群 (graphics processing cluster, GPC)（对于 14 个 SM）或 2 个 GPC（对于 28 个 SM）中；与将 SM 分布在 6 个 GPC 上的分布式 SM (distributed SM) 相比，性能显著下降——当 28 个 SM 访问单个 MP 时大约下降 62%——这是由有限的 GPC 加速比 (GPC speedup) 造成的。为了进一步理解 GPC 加速比的影响，使用了 14 个 SM，并将其流量分布到不同数量的 MP 上。使用分布式 SM 时，对性能的影响相对较小；然而，使用连续 SM 时，随着 MP 目的地数量从 1 增加到 4，性能提高了 218%——因此证明，一部分 GPC 加速比是由空间（即额外的连接性 (connectivity)）提供的，而不完全是由时间（即更高的带宽 (bandwidth)）提供的。
...
> 
观察 #11：由于不对称的片上网络 (NoC) 加速比 (speedup)，在流式多处理器 (SM) 之间对内核 (kernel) 进行负载均衡比在 L2 切片 (L2 slice) 之间进行负载均衡更为关键。
...
> 
GPU 中的片上流量 (on-chip traffic) 由内存流量 (memory traffic)——或 L2 流量 (L2 traffic)——决定，尤其是由内存地址 (memory address) 决定。如果一种内存访问模式 (memory access pattern) 导致某一个内存通道 (memory channel) 被过度利用，则整体内存吞吐量 (memory throughput)（以及系统性能 (system performance)）可能下降——这种行为被称为内存扎堆 (memory camping) [41]。为了防止内存扎堆 (memory camping)，现代 GPU 采用复杂的地址哈希 (address hashing)，以确保对所有 L2 切片 (L2 slice) 的访问是负载均衡 (load-balanced) 的 [42]。为了理解哈希的效果，图 16 针对 V100 GPU 展示了来自 Rodinia 基准测试套件 (Rodinia benchmark suite) [43] 的两个工作负载 (workload)（bfs、gaussian）的内存访问模式 (memory access pattern) 随时间的变化，变化的强度表示发往每个 L2 切片 (L2 slice) 的流量大小。尽管内存访问 (memory access) 的数量随时间变化，但流量在各通道 (channel) 之间的分布仍相对一致。因此，来自 NoC 的片上流量 (on-chip traffic) 的负载均衡 (load-balancing) 并非关键，但需要为每个目的地 (destination) 提供均匀带宽 (uniform bandwidth)，以支持地址哈希 (address hashing)。
...
> 
观察 #12：由于内存流量 (memory traffic) 在内存通道 (memory channels) 之间是负载均衡 (load-balanced) 的，NoC 流量也是负载均衡的。
...
> 
本节探讨 NoC（network-on-chip）特性如何影响 GPU（graphics processing unit）时序侧信道攻击（timing side-channel attack），以及非均匀延迟（non-uniform latency）如何可被利用来促成时序侧信道攻击，并可用作针对时序侧信道攻击的防御机制。
...
> 
在高并行 GPU 架构中，侧信道 (side-channel)（或隐蔽信道 (covert-channel)）攻击通常需要将内核 (kernel) 精心共置 (co-location) 到核心 (core)（或 SM）上，以利用 GPU 的并行性。然而，鉴于现代 GPU 中侧信道 (side-channel) 攻击的威胁日益增加，GPU 供应商正在限制某些被用于侧信道 (side-channel) 攻击的性能计数器 (performance counter) 的可用性——例如，针对先前工作 [44]，没有 root 权限 (root privilege) 就不再能使用性能计数器 (performance counter) [45]。此外，从性能分析器 (profiler) 中获得的信息也受到限制——例如，在最近的 NVIDIA GPU 中，每个 L2 切片 (per-L2 slice) 的性能计数器 (performance counter) 不再可用 [37]，因为只有聚合性能计数器 (aggregate performance counter) 值可用。在这项工作中，我们论证如何利用片上网络 (NoC) 延迟 (latency) 特征来确定核心 (core)（SM）放置 (placement) 信息，并将其用于内核 (kernel) 的共置 (co-location) 放置。如前面在观察 3 (Observation 3) 中所述，L2 缓存 (L2 cache) 访问延迟 (latency) 可作为估计 SM 与 L2 切片 (L2 slice) 之间距离的可靠代理 (proxy)。因此，可以利用皮尔逊相关 (Pearson correlation)（观察 4 (Observation 4)）来逆向推断 GPU 内的核心 (core) 放置 (placement)。这包括确定 SM 在图形处理簇 (GPC) 内的位置，以及 L2 切片 (L2 slice) 在内存分区 (memory partition) 内的位置。因此，当 GPU 侧信道 (side-channel) 攻击 [46]-[48] 需要共置 (co-location) 时，可以利用这些延迟 (latency) 特征。此外，SM 放置 (placement) 可以在 GPU 片上网络 (NoC) 的输入处建立隐蔽信道 (covert channel)，但如果希望在 GPU 片上网络 (NoC) 的输出处（或 L2 的输入处）建立隐蔽信道 (covert channel)，则也可能利用 L2 切片 (L2 slice) 放置 (placement)。
...
> 
图 17：(a) 当唯一缓存行 (cache line) 数量变化时的平均计时（延迟 (latency)），以及 (b) A100 的平方内核 (square kernel) 在不同核心 (core) (SM) 上的平均执行时间 (execution time)。
...
> 
图 18：在采用 (a) 传统（静态）和 (b) 随机线程块 (thread block) 调度的 GPU 上进行 AES 密钥恢复攻击 (key-recovery attack)。正确密钥值已在图中用圆圈标出，且为简单起见，16 个密钥中仅显示了 4 个。
...
> 
启示 #1：NoC 特性分析可用于获取 GPU 中核心 (core) 和/或 L2 切片 (L2 slice) 布局的信息，以用于时序侧信道攻击 (timing side-channel attack)。
...
> 
性能或执行时间与依赖输入的微架构特征 (microarchitecture feature) 之间的相关性，在 GPU 的时序侧信道攻击 (timing side-channel attack) 中通常会被利用 [6], [7]。时序受 L2 延迟 (latency) 影响；对于给定的源 (source)（或 SM）和给定的目的地 (destination)（或 L2），延迟不会改变，但如果内核 (kernel) 在不同核心 (core) 上执行，延迟或时序将会改变。在本节中，我们探讨 NoC 延迟对先前针对 GPU 提出的两种时序侧信道攻击——AES 密钥恢复 (key recovery) [6] 和 RSA 侧信道攻击 [49]——的影响，并讨论 NoC 对攻击与防御机制 (defense mechanism) 两方面的影响。
...
> 
1) AES 密钥恢复 (AES key recovery)：先前工作 [6] 利用唯一缓存行请求数 (number of unique cache line requests) 与执行时间 (execution time) 之间的线性相关性 (linear correlation)，在 GPU 中恢复 AES 密钥。\( {}^{7} \) 对于使用一个线程束 (warp)（含 32 个线程）的合成内核 (synthetic kernel)，取决于内存合并 (memory coalescing) 的程度，延迟 (latency) 与访问的唯一缓存行数 (number of unique cache lines) 成线性比例（图 17(a)）。这些结果与先前工作 [6], [7] 一致，后者在不同 GPU 上展示了类似的线性关系。然而，这种关系仅在利用同一个 SM (streaming multiprocessor, SM)（或核心 (core)）时成立。当分配不同的 SM，而目的地 (destination)（或 L2 切片 (L2 slice)）不变时，线性关系会如图 17(a) 所示针对不同 SM 发生“偏移 (shift)”。
...
> 
\( {}^{7} \) 侧信道攻击 (side channel attack) 的详细描述可在 [6] 中找到。由于篇幅限制，我们仅强调由非均匀互连延迟 (non-uniform interconnect latency) 所导致的关键差异。
...
> 
由于威胁模型 (threat model) 假设攻击者 (attacker) 知道线性关系 (linear relationship)，攻击者将获得 AES 内核 (AES kernel) 在可能密钥 (possible keys) 下的执行时间 (execution time)。但是，除非攻击者知道该内核的确切核心位置 (core location)，唯一缓存行 (unique cache lines) 的范围可能会有显著变化。例如，如果观察到 240 个周期 (cycles) 的延迟 (latency)，唯一缓存行的数量可能从 12 到 18 不等。因此，当 AES 内核在不同核心上执行时，需要知道该特定核心的线性关系；否则，攻击者无法利用时序准确推断唯一缓存行请求的数量并猜测密钥字节 (key byte)。
...
> 
2) RSA 密钥恢复 (RSA key recovery)：先前工作 [49] 展示了如何基于 RSA 密钥 (RSA key) 中 1 的比例与其执行时间 (execution time) 之间的线性关系，推断出该比例。在解密 (decryption) 过程中，RSA 对比特 (bit) 0 执行平方 (square) () 和约减 (reduction) ()，并对比特 1 执行额外的乘法 (multiply) () 和约减 ()。由于每个函数都需要恒定的计算量，处理比特 1 的执行时间约为处理比特 0 的两倍。因此，攻击者 (attacker) 可以利用 RSA 程序执行时间随所执行函数数量的线性增长 (linear increase)，来推断 1 的数量。
...
> 
然而，根据正在执行代码的核心 (core) (SM) 不同，内核 (kernel) 执行时间可能因到内存分区 (memory partition) 的非均匀延迟 (non-uniform latency) 而显著变化。图 17(b) 展示了 A100 上 square () 内核 (kernel) 在不同 SM 上计时的示例。在此示例中，SM 2、16、30、44、58 位于一个分区 (partition) 上，而其他 SM 位于另一个分区 (partition) 上。square 内核 (kernel) [50] 使用两个 SM，并且出于说明目的，一个 SM 固定，而另一个 SM 的位置变化。根据另一个 SM 的位置，计时（延迟）可能显著变化——即，当 SM 位于不同 GPU 分区 (partition) 上时，分区之间增加的延迟以及同步开销 (synchronization overhead) 导致更长的执行时间，最高可达 \( {1.7} \times \)。即使对于位于同一分区 (partition) 内的 SM，非均匀延迟 (non-uniform latency) 也会带来高达 12% 的性能差异。
...
> 
非均匀延迟 (non-uniform latency) 是否可被用于另一种类型的攻击仍有待观察。例如，近期工作 [51] 利用了多跳 (multi-hop) 二维网格 (2D mesh) 网络中的距离以及更高的延迟 (higher latency) 来确定 L2 访问模式 (L2 access pattern)。在 GPU 中，跳数 (hop-count) 未必不同，但延迟特性 (latency characteristics) 有可能被利用来实现新型侧信道攻击 (side-channel attacks)。
...
> 
图 19：采用 (a) 静态和 (b) 随机线程块调度 (thread-block scheduling) 的 RSA 计时攻击 (timing attack)。
...
> 
尽管先前的侧信道攻击 (side-channel attack) 已在真实 GPU 系统上得到演示，但它们需要收集大量数据样本 (data sample)，不过每次执行工作负载 (workload) 时，核心 (core)（或流式多处理器 (streaming multiprocessor, SM)）的分配都相同——也就是说，由于分配的是相同的 SM，观察到的延迟 (latency) 相同——并且观察不到非均匀延迟 (non-uniform latency)。GPU 中的线程块 (thread block)（或 CTA）调度负责将内核 (kernel) 分配给核心，实际上是一种静态调度 (static scheduling)。因此，我们提出使用随机分配 (random assignment) 的动态调度 (dynamic scheduling)，因为对于每次内核执行 (kernel execution)，线程块最初被映射到随机 SM 上，以防止侧信道攻击。随机线程块调度 (random thread-block scheduling) 实际上是一种随机种子调度 (random seed scheduling)，因为核心的分配从随机核心开始。
...
> 
1) AES 密钥恢复 (AES key recovery)：随机线程块调度 (random thread block scheduling) 对 AES 密钥恢复的影响如图 18 所示，其中展示了 \( x \) 轴上不同的可能密钥值以及皮尔逊相关系数 (Pearson correlation) 值。皮尔逊相关系数用于比较时序以及唯一缓存行请求数量的分布，并且当正确的密钥字节被猜中时，会获得高相关值。在传统静态调度 (static scheduling) 下，图 18(a) 展示了如何恢复 AES 密钥。\( {}^{8} \) 然而，在所提出的随机线程块调度下，高皮尔逊相关系数消失了（图 18(b)），因为随机调度导致 AES 的执行时间基于到 L2 缓存片 (L2 cache slices) 的非均匀延迟 (non-uniform latency) 而变得不确定。
...
> 
2) RSA 密钥恢复 (RSA key recovery)：随机线程块调度 (random thread block scheduling) 对 RSA 密钥恢复的影响如图 19 所示。在静态线程块调度 (static thread-block scheduling) 下，“1”的数量与执行时间 (execution time) 之间存在线性关系 (linear relationship)；然而，当 RSA 代码在不同 SM (SM) 上执行时，该线性关系会根据核心位置 (core location) 发生偏移。相比之下，当使用随机线程块调度时，如图 19(b) 所示，该线性关系变得噪声很大。RSA 计时 (RSA timing) 中的单个内核 (individual kernel) 会因放置位置 (placement) 而变化，并导致总体执行时间出现显著变化。因此，如果攻击者 (attacker) 拥有测量得到的计时 (measured timing)，“1”的数量的范围可能显著变化——例如，在执行时间为 \( 2 \times  {10}^{9} \) 个周期时，“1”的数量可能在 416 到 1920 之间变化。
...
> 
启示 #2：非均匀 L2 延迟 (non-uniform L2 latency) 不会从根本上改变时序侧信道攻击 (timing side-channel attacks)，但核心之间的时序差异可能影响侧信道攻击行为。
...
> 
\( {}^{8} \) 由于篇幅限制，仅展示了16个密钥 (key) 中的前四个，但其余密钥 (key) 遵循相同的模式。
...
> 
图 20：吞吐量处理器 (throughput processors) 中多对少对多 (many-to-few-to-many) 的通信模式。(a) 对分带宽 (bisection bandwidth) \( \left( {B{W}_{{NoC} - {B}_{c}}}\right) \) 和 (b) 内存带宽 (memory bandwidth) \( \left( {B{W}_{MEM}}\right) \) 以及接口带宽 (interface bandwidth) \( \left( {B{W}_{{NoC} - {MEM}}}\right) \) 被突出显示。
...
> 
启示 #3：随机线程块调度 (random thread block scheduling) 可利用非均匀延迟 (non-uniform latency) 来降低 GPU 中时序侧信道攻击 (timing side-channel attacks) 的影响。
...
> 
随机化 (randomization) 通常用于防止在侧信道攻击 (side-channel attacks) 中收集有用的时序测量 (timing measurements) [52]-[54]。然而，随机化常常被添加到硬件中（例如，随机化合并逻辑 (coalescing logic) [52]），这会引入一定复杂度，并可能降低性能。相比之下，我们的随机线程块调度 (random thread-block scheduling) 实际上是一种随机种子调度 (random seed scheduling)，不会引入额外的硬件复杂度。尽管随机线程块调度能够降低先前提出的时序侧信道攻击 (timing side-channel attacks) 的影响，但所提出的解决方案是否能完全安全地抵御各种时序侧信道攻击仍有待观察。
...
> 
本节中，我们探讨片上网络 (NoC) 分析对 GPU 片上网络 (NoC) 本身的启示。更重要的是，我们重新审视先前 GPU NoC [28] 中识别出的 GPU NoC 瓶颈 (bottleneck)，并表明此前假定的 NoC 瓶颈 (bottleneck) 不能恰当建模 NoC 带宽 (bandwidth) 对整体内存 (memory) 和 L2 带宽 (bandwidth) 的影响。
...
> 
GPU 中的片上网络 (network-on-chip, NoC) 由请求网络和回复网络组成（图 20(a)），具有多对少对多 (many-to-few-to-many) 流量模式 [28]——其中少数内存控制器 (memory controller) 相较众多核心成为性能瓶颈。由于高吞吐量 (high-throughput) 工作负载的内存访问通常是读操作 (Read operation)，其读请求大小较小，但回复数据大小较大，因此回复带宽（图 20(b) 中的 BW \( {}_{\text{ noc-mem }} \)）被认定为吞吐处理器 (throughput processor) 的 NoC 瓶颈 [28], [30], [31], [33]。当拥塞在回复网络的 NoC-MEM 接口处积累时，拥塞反压 (backpressure) 不仅影响内存带宽利用率，还影响请求网络。然而，我们重新审视回复带宽瓶颈，并评估其对内存带宽利用率的影响。使用与先前工作 [28] 相似的配置，图 21(a) 绘制了执行内存密集型 (memory-intensive) 合成内核 (synthetic kernel) 时单个内存通道 (memory channel) 的内存利用率随时间的变化。内存带宽达到最大带宽，但该带宽无法持续，因为平均内存带宽利用率显著更低——约为 20% 利用率——这是由于回复带宽接口拥塞阻塞了内存系统，并向请求 NoC 施加反压。然而，在真实 GPU 上的评估，如前文图 9(a) 所示，在不同 GPU 上均提供了高带宽利用率，超过 85% 利用率。因此，模拟器 (simulator) 的 NoC 建模不一定能代表真实 GPU 系统
...
> 
图 21：在基于仿真的评估中，由回复带宽瓶颈引发的背压导致的内存通道利用率波动。
...
> 
除了内存带宽 (memory bandwidth) 之外，L2 互连带宽 (L2 fabric bandwidth) 或提供给 L2 的片上带宽 (on-chip bandwidth) 也会受到片上网络 (NoC) 带宽的影响。正如前面图 9 所示，在近期的图形处理器 (GPU) 中，L2 带宽 (L2 bandwidth) 超过总体内存带宽 (overall memory bandwidth)；然而，使用相同配置的仿真也表明，L2 带宽低于最大内存带宽 (maximum memory bandwidth)。\( {}^{9} \) 因此，GPU 中的 NoC 需要被设计（和建模）成不会对内存带宽和 L2 带宽两者造成瓶颈。
...
> 
启示 #4：在真实 GPU 中，片上网络 (NoC) 不会成为内存（或 L2）带宽的瓶颈，但对 NoC 的不准确假设可能限制整体系统带宽。
...
> 
\( {}^{9} \) 请注意，这并非来自模拟器 (simulator) 中对 L2 缓存 (L2 cache) 的建模，而是来自对片上网络 (NoC) 的建模/假设 (modeling/assumption)。
...
> 
启示 #5：对于片上网络 (NoC)，需要考虑片上带宽层次 (on-chip bandwidth hierarchy)，以确保它不会成为整个系统的瓶颈；特别是，除了二分带宽 (bisection bandwidth) 之外，还需要合理配置接口带宽 (interface bandwidth)。
...
> 
吞吐量处理器 (throughput processor)（如GPU）对带宽敏感，应在不同目的地或内存分区 (memory partition) 之间提供均匀带宽 (uniform bandwidth)（第IV-A节）。然而，如果假设采用二维网格 (2D mesh) 等多跳 (multi-hop) 网络作为NoC拓扑，那么提供均匀带宽便是一项挑战，如图23(a)所示。\( {}^{10} \) 基于核心节点的物理位置，某些节点将获得高得多的吞吐量，最高可达 \( {2.4} \times \) 。如果采用全局公平仲裁 (globally fair arbitration)（如基于年龄的仲裁 (age-based arbitration) [61], [62]），那么在二维网格 (2D mesh) 拓扑上可以实现全局公平带宽 (global fair bandwidth)，但代价是流控 (flow control) 的额外复杂性。此外，在二维网格 (2D mesh) 等扁平拓扑 (flat topology) 中，在NoC中提供任何必要的加速 (speed-up) 都是一项挑战。近期工作 [34], [35] 利用层次化交叉开关 (hierarchical crossbar) 作为基线NoC拓扑，其不一定具有与二维网格 (2D mesh) NoC相同的局限性。虽然层次化交叉开关 (hierarchical crossbar) 可以提供更多带宽，但NoC到内存 (memory)（以及L2）的带宽需要为GPU架构探索 (architectural exploration) 仔细配置。近期的一项GPU模拟器探索 (Accel-Sim [36]) 讨论了L2带宽差异 (discrepancy) 如何可能解释其对现代GPU建模中的一些不准确性。因此，需要考虑整个系统的带宽层次结构 (bandwidth hierarchy)，而不仅仅是NoC带宽，并在仿真中对其进行适当建模。
...
> 
图 22：内存带宽 (memory bandwidth) 与 NoC-Mem 接口带宽 (NoC-Mem interface bandwidth) 的比较，以不同符号建模，代表不同的基于仿真的先前工作 (simulation-based prior work) [14], [15], [17], [28]- [32], [58], [59]。
...
> 
图 23：多跳 (multi-hop) 二维网格 (2D mesh) 拓扑架构中出现的带宽不公平现象，分别对应 (a) 轮询仲裁 (round-robin arbitration) 与 (b) 全局公平、基于年龄的仲裁 (age-based arbitration)。
...
> 
启示 #6：多跳拓扑 (multi-hop topologies)（例如二维网格 (2D mesh)）在为各节点 (node) 提供均匀带宽 (uniform bandwidth) 支持方面存在挑战，无论其放置位置 (placement) 如何；同时，也难以为片上网络 (NoC) 提供所需的任何加速比 (speedup)。
...
> 
这项工作对现代 GPU 的片上网络 (network-on-chip, NoC) 进行了详细分析，涉及其延迟 (latency) 和带宽 (bandwidth)。特别地，非均匀延迟 (non-uniform latency) 会因核心 (cores) 和 L2 切片 (L2 slices) 的放置位置而产生，而片上带宽 (on-chip bandwidth) 则大致均匀。然而，具有多个“分区” (partitions) 的更大 GPU 会在带宽和性能方面引入额外的非均匀性。基于对 NoC 的表征 (characterization)，我们提出了一些启示，包括其对 GPU 的时序侧信道攻击 (timing side-channel attacks) 以及 NoC 架构设计 (NoC architecture designs) 的影响。
...
> 
\( {}^{10} \) 仅网络模拟 (network-only simulation) [60] 使用了 \( 6 \times 6 \) 二维网格 (2D mesh)，随机流量 (random traffic) 从 30 个计算节点 (compute nodes) 发送到位于边缘的 6 个内存控制器 (memory controllers)，并采用维序路由 (dimension-ordered routing) 和轮转仲裁 (round-robin arbitration)。吞吐量 (throughput) 为零的节点是内存节点 (memory nodes)。
...
> 
我们感谢匿名审稿人 (anonymous reviewers) 提出的富有洞见的意见，并感谢 Jo Sangheon 收集 NoC 流量分布数据。本工作部分由 IITP-RS202300228255、NRF-2023R1A2C200422912 和 IITP-RS202400402898 资助。
...
> 
[1] J. L. Hennessy 和 D. A. Patterson，“计算机体系结构的新黄金时代”，《ACM通讯》，第62卷，第2期，第48-60页，2019年。
...
> 
[2] A. Vaswani, N. Shazeer, N. Parmar, J. Uszkoreit, L. Jones, A. N. Gomez, L. Kaiser, and I. Polosukhin, "注意力就是你所需要的一切 (Attention is all you need)," in 第31届国际神经信息处理系统大会论文集 (Proceedings of the 31st International Conference on Neural Information Processing Systems), 2017, p. 6000-6010.
...
> 
[3] J. Devlin, M.-W. Chang, K. Lee, and K. Toutanova, "Bert：用于语言理解的深度双向变换器 (transformer) 预训练," 2019.
...
> 
[4] D. B. Kirk 和 W.-m. W. Hwu，《大规模并行处理器编程：实践方法》，第1版。美国加利福尼亚州旧金山：Morgan Kaufmann Publishers Inc.，2010年。
...
> 
[5] W. Dally 和 B. Towles，《互连网络原理与实践》(Principles and Practices of Interconnection Networks)。Morgan Kaufmann Publishers Inc.，2003年。
...
> 
[6] Z. H. Jiang、Y. Fei 和 D. Kaeli，“针对 GPU 的完整密钥恢复时序攻击 (A complete key recovery timing attack on a gpu)，”载于 2016 IEEE 国际高性能计算机体系结构研讨会 (International Symposium on High Performance Computer Architecture, HPCA)，2016，pp. 394-405。
...
> 
[8] C. Luo、Y. Fei 和 D. Kaeli，“GPU 上 RSA 的侧信道时序攻击 (side-channel timing attack)”，ACM Transactions on Architecture and Code Optimization (TACO)，vol. 16, no. 3, pp. 1-18, 2019.
...
> 
[9] Y. Wang, R. Paccagnella, Z. Gang, W. Vasquez, D. Kohlbrenner, H. Shacham, and C. Fletcher, "Gpu.zip：关于基于硬件的图形数据压缩的侧信道 (side-channel) 影响," 载于 2024 IEEE Symposium on Security and Privacy (SP), 2024, pp. 84-84.
...
> 
[10] S. B. Dutta, H. Naghibijouybari, A. Gupta, N. Abu-Ghazaleh, A. Marquez, 和 K. Barker，“GPU 盒中的间谍：针对多 GPU (multi-GPU) 系统的隐蔽信道与侧信道攻击 (covert and side channel attacks)，”载于第50届年度计算机体系结构国际研讨会 (Annual International Symposium on Computer Architecture) 论文集，2023，第1-13页。
...
> 
[11] Z. H. Jiang、Y. Fei 和 D. Kaeli，“一种针对 GPU 的新型侧信道时序攻击 (side-channel timing attack on GPUs)”，载于 Proceedings of the on Great Lakes Symposium on VLSI 2017，2017，第 167-172 页。
...
> 
[12] NVIDIA，“H100白皮书 (H100 Whitepaper)，”NVIDIA Corporation，技术报告 (Tech. Rep.)，2022，https://resources.nvidia.com/en-us-tensor-core。
...
> 
[13] CUDA C++ 编程指南 (CUDA C++ Programming Guide)，NVIDIA，https://docs.nvidia.com/ cuda/cuda-c-programming-guide/index.html。
...
> 
[14] T. G. Rogers、M. O'Connor 和 T. M. Aamodt，“缓存感知波前调度 (Cache-conscious wavefront scheduling)”，载于《2012年第45届IEEE/ACM国际微架构研讨会论文集》。IEEE计算机学会，2012年，第72-83页。
...
> 
[15] A. Sethia、D. A. Jamshidi 和 S. Mahlke，“Mascar：通过减少内存中途停靠 (memory pitstops) 来加速图形处理器 (GPU) 线程束 (warp)，”载于 2015 年第 21 届 IEEE 高性能计算机体系结构 (High Performance Computer Architecture) 国际研讨会 (HPCA)，2015，第 174-185 页。
...
> 
[16] T. G. Rogers、M. O'Connor 和 T. M. Aamodt，“分歧 (divergence) 感知的线程束 (warp) 调度”，载于 2013 年第 46 届 IEEE/ACM 微架构 (Microarchitecture) 国际研讨会 (MICRO) 年会，2013，第 99-110 页。
...
> 
[17] M. Lee、G. Kim、J. Kim、W. Seo、Y. Cho 和 S. Ryu，“ipaws：面向通用图形处理器 (GPGPU) 的基于指令发射模式 (instruction-issue pattern) 的自适应线程束 (warp) 调度”，载于 2016 IEEE 高性能计算机体系结构国际研讨会 (International Symposium on High Performance Computer Architecture, HPCA)，2016，第 370-381 页。
...
> 
[18] S. Jain, I. Baek, S. Wang, 和 R. Rajkumar，“分片 GPU (Fractional GPUs)：面向 GPU 的基于软件的计算和内存带宽预留”，载于 2019 IEEE 实时与嵌入式技术与应用研讨会 (RTAS)，2019 年，第 29-41 页。
...
> 
[19] NVIDIA，“NVIDIA Pascal架构白皮书 (NVIDIA Pascal Architecture Whitepaper)”，NVIDIA公司，技术报告 (Tech. Rep.)，2012，https://images.nvidia.com/content/pdf/tesla/ whitepaper/pascal-architecture-whitepaper.pdf。
...
> 
[20] NVIDIA，《NVIDIA Tesla V100 GPU架构》，NVIDIA公司，技术报告，2017年，https://images.nvidia.com/content/volta-architecture/pdf/volta-architecture-whitepaper.pdf。
...
> 
[21] NVIDIA，"NVIDIA Ampere 架构白皮书"，NVIDIA 公司，技术报告，2020，https://images.nvidia.com/aem-dam/en-zz/ Solutions/data-center/nvidia-ampere-architecture-whitepaper.pdf。
...
> 
[22] Z. Jia、M. Maggioni、B. Staiger 和 D. P. Scarpazza，“通过微基准测试 (microbenchmarking) 剖析 NVIDIA Volta GPU 架构，”arXiv 预印本 arXiv:1804.06826，2018。
...
> 
[23] H. Abdelkhalik, Y. Arafa, N. Santhi, and A.-H. Badawy, "通过微基准测试 (microbenchmarking) 和指令级分析 (instruction-level analysis) 揭秘 NVIDIA Ampere 架构," 2022. [Online]. 可获取于: https://arxiv.org/ abs/2208.11174
...
> 
[24] S. Dublish, V. Nagarajan, and N. Topham, "评估并缓解 GPU 中跨存储层次结构 (memory hierarchy) 的带宽瓶颈 (bandwidth bottlenecks)," in 2017 IEEE International Symposium on Performance Analysis of Systems and Software (ISPASS), 2017, pp. 239-248.
...
> 
[25] A. Li, S. L. Song, J. Chen, X. Liu, N. Tallent, and K. Barker, “Tartan：通过多 GPU 基准测试套件评估现代 GPU 互连 (interconnect)，” 载于 2018 IEEE 工作负载表征国际研讨会 (International Symposium on Workload Characterization, IISWC), 2018, pp. 191-202.
...
> 
[26] A. Li、S. L. Song、J. Chen、J. Li、X. Liu、N. R. Tallent 和 K. J. Barker，“评估现代 GPU 互连 (interconnect)：PCIe、NVLink、NV-SLI、NVSwitch 和 GPUDirect”，IEEE 并行与分布式系统汇刊 (IEEE Transactions on Parallel and Distributed Systems)，第 31 卷，第 1 期，第 94-110 页，2019。
...
> 
[27] C. Lutz, S. Breß, S. Zeuch, T. Rabl, 和 V. Markl，“提高音量：利用快速互连 (fast interconnects) 在 GPU 上处理大数据 (large data)，” 载于《2020 年 ACM SIGMOD 数据管理国际会议 (International Conference on Management of Data) 论文集》，2020，第 1633-1649 页。
...
> 
[28] A. Bakhoda、J. Kim 和 T. M. Aamodt，“面向众核加速器 (manycore accelerators) 的吞吐高效片上网络 (on-chip networks)”，载于 2010 年第 43 届 IEEE/ACM 微架构国际研讨会 (Annual IEEE/ACM International Symposium on Microarchitecture)，2010，第 421-432 页。
...
> 
[29] X. Cheng, Y. Zhao, H. Zhao, and Y. Xie，〈分组泵 (Packet pump)：克服通用图形处理器 (GPGPU) 片上互连 (on-chip interconnects) 中的网络瓶颈 (network bottleneck)〉，载于《第 55 届设计自动化会议 (Design Automation Conference) 年会论文集》，2018，第 1–6 页。
...
> 
[30] H. Jang, J. Kim, P. Gratz, K. H. Yum, and E. J. Kim，“面向通用图形处理器 (GPGPU) 的带宽高效片上互连 (on-chip interconnect) 设计”，载于第52届设计自动化会议 (Design Automation Conference) 论文集，2015年，第1-6页。
...
> 
[31] H. Kim、J. Kim、W. Seo、Y. Cho 和 S. Ryu，“在 GPGPU 中提供经济高效的片上网络带宽 (on-chip network bandwidth)，” 载于 2012 IEEE 第 30 届国际计算机设计会议 (International Conference on Computer Design, ICCD)，2012，第 407-412 页。
...
> 
[32] X. Zhao, S. Ma, Y. Liu, L. Eeckhout, and Z. Wang, “面向通用图形处理器 (GPGPU) 的低成本无冲突片上网络 (NoC)，” 载于第53届设计自动化年会 (Design Automation Conference) 论文集, 2016, pp. 1-6.
...
> 
[33] K. H. Kim, R. Boyapati, J. Huang, Y. Jin, K. H. Yum 和 E. J. Kim，“在 GPGPU 架构中利用数据冗余 (data redundancy) 的分组合并 (packet coalescing)”，载于《国际超级计算会议 (International Conference on Supercomputing) 论文集》，2017，第 1–10 页。
...
> 
[34] X. Zhao、M. Jahre、Y. Tang、G. Zhang 和 L. Eeckhout，“Nuba：非均匀带宽 GPU (Nonuniform Bandwidth GPUs)”，载于《第28届 ACM 编程语言和操作系统体系结构支持国际会议 (International Conference on Architectural Support for Programming Languages and Operating Systems) 论文集》，第2卷，2023年，第544–559页。
...
> 
[35] X. Zhao, A. Adileh, Z. Yu, Z. Wang, A. Jaleel, and L. Eeckhout, "自适应内存侧末级 GPU 缓存 (Adaptive memory-side last-level GPU caching)," 载于第 46 届国际计算机体系结构研讨会 (International Symposium on Computer Architecture) 论文集, 2019, 第 411-423 页.
...
> 
M. Khairy、Z. Shen、T. M. Aamodt 和 T. G. Rogers，“Accel-sim：一种用于经过验证的图形处理器 (GPU) 建模的可扩展仿真框架”，载于 2020 ACM/IEEE 第 47 届国际计算机体系结构研讨会 (ISCA)，2020，第 473-486 页。
...
> 
[37] NVIDIA，"Profiler 用户指南 (Profiler User's Guide)"，NVIDIA 公司，技术报告 (Tech. Rep.)，2024，https://docs.nvidia.com/cuda/profiler-users-guide/index.html#nvprof。
...
> 
[38] Techpowerup，“NVIDIA GV100 GPU 规格说明,” https://www.techpowerup.com/gpu-specs/nvidia-gv100.g809.
...
> 
[39] K. Pearson，〈关于双亲情形下的回归与遗传的笔记〉，《伦敦皇家学会会刊》，第 58 卷，1895 年。
...
> 
[40] Locuza，“Nvidia的Ada系列产品线、配置、估计芯片尺寸及与其他芯片的比较”，2022年，https://locuza.substack.com/p/nvidias-ada-lineup-configurations?utm_ source=%2Fsearch%2FCPC&utm_medium=reader2。
...
> 
[41] A. M. Aji、M. Daga 和 W.-c. Feng，“Bounding the effect of partition camping in gpu kernels,” 载于 Proceedings of the 8th ACM International Conference on Computing Frontiers，2011年，第1-10页。
...
> 
[42] Z. Zhang, T. Allen, F. Yao, X. Gao 和 R. Ge，“用于走私的隧道：完全逆向工程 GPU 转译后备缓冲器 (TLB) 以挑战 NVIDIA 多实例 GPU (MIG) 的隔离保证”，载于《2023 年 ACM SIGSAC 计算机与通信安全会议论文集》(Proceedings of the 2023 ACM SIGSAC Conference on Computer and Communications Security)，系列 CCS '23。美国纽约州纽约市：计算机协会 (Association for Computing Machinery)，2023，第 960-974 页。[在线]。可从：https://doi.org/10.1145/3576915.3616672
...
> 
[43] S. Che、M. Boyer、J. Meng、D. Tarjan、J. W. Sheaffer、S.-H. Lee 和 K. Skadron，“Rodinia：一个面向异构计算 (heterogeneous computing) 的基准测试套件 (benchmark suite)”，载于 2009 IEEE 国际工作负载表征研讨会 (International Symposium on Workload Characterization, IISWC)，2009，第 44-54 页。
...
> 
[44] H. Naghibijouybari、A. Neupane、Z. Qian 和 N. Abu-Ghazaleh，“渲染不安全：GPU 侧信道攻击 (side channel attacks) 切实可行，”载于 2018 年 ACM SIGSAC 计算机与通信安全 (computer and communications security) 会议 (conference) 论文集 (Proceedings)，2018，第 2139-2153 页。
...
> 
[45] NVIDIA，“NVIDIA 对‘Rendered Insecure: GPU Side Channel Attacks are Practical’的回应”，NVIDIA Corporation，技术报告 (Tech. Rep.)，2018，https://nvidia.custhelp.com/app/answers/detail/a_id/4738/~/security-notice%3A-nvidia-response-to-.
...
> 
[46] H. Naghibijouybari、K. N. Khasawneh 和 N. Abu-Ghazaleh，“在通用图形处理器 (GPGPU) 上构建与刻画隐蔽信道 (covert channels)，”载于 2017 年第 50 届 IEEE/ACM 微架构国际研讨会 (International Symposium on Microarchitecture, MICRO)，2017。
...
> 
[47] J. Ahn、J. Kim、H. Kasan、L. Delshadtehrani、W. Song、A. Joshi 和 J. Kim，“GPU 中基于片上网络 (network-on-chip) 微架构 (microarchitecture) 的隐蔽信道 (covert channel)，” 见 MICRO-54：第54届 IEEE/ACM 年度国际微架构 (microarchitecture) 研讨会，2021，第565-577页。
...
> 
[48] J. Wei, Y. Zhang, Z. Zhou, Z. Li, and M. A. Al Faruque, “Leaky dnn：利用 GPU 上下文切换侧信道 (context-switching side-channel) 窃取深度学习模型 (deep-learning model) 秘密,” in 2020 50th Annual IEEE/IFIP International Conference on Dependable Systems and Networks (DSN), 2020, pp. 125-137.
...
> 
[49] Y. Wang 和 G. E. Suh，“面向片上网络的高效时序信道保护 (Efficient timing channel protection for on-chip networks)”，载于 2012 IEEE/ACM 第六届国际片上网络研讨会 (Sixth International Symposium on Networks-on-Chip)，2012 年，第 142-151 页。
...
> 
[50] NVIDIA，“CUDA 代码示例 (CUDA Code Samples)”，2024，https://developer.nvidia.com/ cuda-code-samples。
...
> 
[51] F. Mahmud, S. Kim, H. S. Chawla, E. J. Kim, C.-C. Tsai, and A. Muza-hid, "骑士的攻击：非均匀缓存侧信道攻击 (Attack of the knights: Non uniform cache side channel attack)," 载于第39届年度计算机安全应用会议 (Annual Computer Security Applications Conference) 论文集, 2023, pp. 691-703.
...
> 
[52] G. Kadam, D. Zhang, and A. Jog, "Bcoal：基于分桶的内存合并，用于高效且安全的 GPU"，in 2020 IEEE 国际高性能计算机体系结构研讨会 (HPCA)，2020, pp. 570-581.
...
> 
[53] E. Karimi、Y. Fei 和 D. Kaeli，“针对 GPU 上时序侧信道攻击 (timing side-channel attack) 的硬件/软件混淆 (hardware/software obfuscation)”，载于 2020 IEEE 硬件导向安全与信任国际研讨会 (International Symposium on Hardware Oriented Security and Trust, HOST)，2020，第 122-131 页。
...
> 
[54] G. Kadam、D. Zhang 和 A. Jog，“Rcoal：通过基于子线程束 (subwarp) 的随机化合并 (coalescing) 技术缓解 GPU 时序攻击 (timing attack)，”载于 2018 IEEE 国际高性能计算机体系结构研讨会 (International Symposium on High Performance Computer Architecture, HPCA)，2018，第 156-167 页。
...
> 
[55] E. D. Lazowska, J. Zahorjan, G. S. Graham, 和 K. C. Sevcik, 定量系统性能：使用排队网络模型的计算机系统分析 (Quantitative system performance: computer system analysis using queueing network models). Prentice-Hall, Inc., 1984.
...
> 
[56] M. D. Hill，“计算机系统性能的另外三种模型 (Three other models of computer system performance)，” arXiv 预印本 (arXiv preprint) arXiv:1901.02926，2019 年。
...
> 
[57] J. Kim, S. Cho, M. Rhu, A. Bakhoda, T. M. Aamodt, and J. Kim, "高吞吐量处理器的片上网络 (network-on-chip) 中的带宽瓶颈 (bandwidth bottleneck)，" 载于《ACM 并行体系结构与编译技术国际会议论文集》，2020年，第157-158页。
...
> 
[58] J. Kloosterman、J. Beaumont、M. Wollman、A. Sethia、R. Dreslinski、T. Mudge 和 S. Mahlke，“Warppool：通过线程束间合并 (inter-warp coalescing) 为吞吐量处理器 (throughput processors) 共享请求”，载于 2015 年第 48 届 IEEE/ACM 国际微架构年度研讨会 (Annual International Symposium on Microarchitecture, MICRO)，2015 年 12 月，第 433-444 页。
...
> 
[59] X. Chen、L.-W. Chang、C. I. Rodrigues、J. Lv、Z. Wang 和 W.-M. Hwu，“面向节能 GPU 计算的自适应缓存管理 (Adaptive Cache Management for Energy-Efficient GPU Computing)”，载于 2014 年第 47 届 IEEE/ACM 微架构国际研讨会 (Annual IEEE/ACM International Symposium on Microarchitecture)，2014 年，第 343–355 页。
...
> 
[60] N. Jiang, D. U. Becker, G. Michelogiannakis, J. Balfour, B. Towles, D. E. Shaw, J. Kim, and W. J. Dally，“一种详细且灵活的周期精确型片上网络 (network-on-chip) 模拟器”，载于 2013 IEEE 系统与软件性能分析国际研讨会 (ISPASS)，2013 年，第 86-96 页。
...
> 
[61] M. M. Lee, J. Kim, D. Abts, M. Marty, and J. W. Lee, "基于概率距离的仲裁：为众核CMP提供服务均等性," in 2010 43rd Annual IEEE/ACM International Symposium on Microarchitecture, 2010, pp. 509-519.
...
> 
[62] D. Abts 和 D. Weisser，“大规模基数 k-ary n-cubes 中基于年龄的分组仲裁 (packet arbitration)”，载于 2007 年 ACM/IEEE 超级计算会议论文集，2007 年，第 1-11 页。
